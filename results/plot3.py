...
ps = np.genfromtxt(sys.argv[3], delimiter=',', skip_header=1)
psjf = np.genfromtxt(sys.argv[4], delimiter=',', skip_header=1)
...
# plt.xlim(0.15,3)
plt.grid(axis="y")
p1, p10, p100, p1000 = plt.plot(fifo[:,1]/1e6, fifo[:,2], 'r', sjf[:,1]/1e6, sjf[:,2], 'g', ps[:,1]/1e6, ps[:,2], 'b',psjf[:,1]/1e6, psjf[:,2], 'y', linewidth=3)
p2, p20, p200, p2000 = plt.plot(fifo[:,1]/1e6, fifo[:,7], 'r:', sjf[:,1]/1e6, sjf[:,7], 'g:',ps[:,1]/1e6, ps[:,7], 'b:',psjf[:,1]/1e6, psjf[:,7], 'y:', linewidth=3)

# plt.xticks(np.arange(0, 3, 0.1))
# ax.set_xticklabels(np.arange(0, 3, 0.1))

plt.legend((p1, p2, p10, p20, p100, p200, p1000, p2000), ('FIFO-mean', 'FIFO-99.9', "SJF-mean", 'SJF-99.9', "PS-mean", 'PS-99.9', 'PSJF-mean', 'PSJF-99.9'))
